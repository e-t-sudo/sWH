...
if(isset($_POST['checkout'])){
	...
	$card_name = $_POST['card_name'];
		$cart_result = mysqli_query(mysqli_connect("localhost", "root", "", "newbase"), "select * from carts where cart_id = ".$customer_id);
		$cart_total = 0;
		//calculation of cart_total starts
		while($row = mysqli_fetch_assoc($cart_result)){
				$prod_id = $row['prod_id'];
				$activation_period = $row['activation_period'];
				$prod_array = mysqli_fetch_assoc(mysqli_query(mysqli_connect("localhost", "root", "", "newbase"), "select * from products where prod_id = ".$prod_id));
